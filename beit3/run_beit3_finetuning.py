...
def main(args, ds_init):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            # Log validation metrics vào Tensorboard
            if log_writer is not None:
                # Log general performance metric
                log_writer.update(acc=test_stats[task_key], head="perf", step=epoch)
                
                # Log retrieval-specific metrics nếu là retrieval task
                if args.task in ["flickr30k", "coco_retrieval"]:
                    # Log evaluation loss
                    if 'loss' in test_stats:
                        log_writer.update(head="eval", loss=test_stats['loss'], step=epoch)
                    
                    # Log Text-to-Image retrieval metrics
                    log_writer.update(
                        head="eval/text_to_image",
                        r1=test_stats.get('tr_r1', 0),
                        r5=test_stats.get('tr_r5', 0),
                        r10=test_stats.get('tr_r10', 0),
                        step=epoch
                    )
                    
                    if 'tr_mean_rank' in test_stats:
                        log_writer.update(
                            head="eval/text_to_image",
                            mean_rank=test_stats['tr_mean_rank'],
                            median_rank=test_stats['tr_median_rank'],
                            step=epoch
                        )
                    
                    # Log Image-to-Text retrieval metrics
                    log_writer.update(
                        head="eval/image_to_text",
                        r1=test_stats.get('ir_r1', 0),
                        r5=test_stats.get('ir_r5', 0),
                        r10=test_stats.get('ir_r10', 0),
                        step=epoch
                    )
                    
                    if 'ir_mean_rank' in test_stats:
                        log_writer.update(
                            head="eval/image_to_text",
                            mean_rank=test_stats['ir_mean_rank'],
                            median_rank=test_stats['ir_median_rank'],
                            step=epoch
                        )
                    ...
                    log_writer.update(
                        head="eval",
                        average_score=test_stats.get('average_score', 0),
                        step=epoch
                    )
