...
class RetrievalHandler(TaskHandler):
    ...
    def eval_batch(self, model, image, language_tokens, padding_mask, image_id):
        ...
        # Tính eval loss
        vision_cls_norm = vision_cls / vision_cls.norm(dim=-1, keepdim=True)
        language_cls_norm = language_cls / language_cls.norm(dim=-1, keepdim=True)
        
        # Sử dụng logit_scale = 1.0 cho evaluation
        logit_scale = torch.tensor(1.0).to(vision_cls.device)
        loss, _, _ = self.criterion(vision_cls_norm, language_cls_norm, logit_scale)
    ...
    def after_eval(self, **kwargs):
        ...
        # Text-to-Image Mean Rank và Median Rank
        tr_ranks = []
        for i in range(len(iids)):
            # Sắp xếp scores từ cao đến thấp để tìm rank
            sorted_indices = scores[i].argsort(descending=True)
            # Tìm vị trí của correct image (rank bắt đầu từ 1)
            rank = (sorted_indices == i).nonzero(as_tuple=True)[0].item() + 1
            tr_ranks.append(rank)
        
        tr_ranks_tensor = torch.tensor(tr_ranks, dtype=torch.float32)
        tr_mean_rank = tr_ranks_tensor.mean().item()
        tr_median_rank = tr_ranks_tensor.median().item()
        ...
        # Image-to-Text Mean Rank và Median Rank
        ir_ranks = []
        for i in range(len(tiids)):
            # Tìm image index tương ứng
            img_idx = (iids == tiids[i]).nonzero(as_tuple=True)[0].item()
            # Sắp xếp scores từ cao đến thấp
            sorted_indices = scores[:, i].argsort(descending=True)
            # Tìm vị trí của correct image (rank bắt đầu từ 1)
            rank = (sorted_indices == img_idx).nonzero(as_tuple=True)[0].item() + 1
            ir_ranks.append(rank)
        
        ir_ranks_tensor = torch.tensor(ir_ranks, dtype=torch.float32)
        ir_mean_rank = ir_ranks_tensor.mean().item()
        ir_median_rank = ir_ranks_tensor.median().item()

        # Lấy eval loss từ metric_logger
        eval_loss = self.metric_logger.loss.global_avg
        ...
        eval_result = {
            ...
            "tr_mean_rank": tr_mean_rank,
            "tr_median_rank": tr_median_rank,
            "ir_r10": ir_r10.item() * 100.0, 
            "ir_r5": ir_r5.item() * 100.0, 
            "ir_r1": ir_r1.item() * 100.0,
            "ir_mean_rank": ir_mean_rank,
            "ir_median_rank": ir_median_rank,
