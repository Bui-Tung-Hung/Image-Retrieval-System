...
def main(args):
    ...
    if args.use_lora:
        if is_master(args):
            logging.info('=' * 60)
            logging.info('Applying LoRA to text encoder...')
            logging.info(f'LoRA config: r={args.lora_r}, alpha={args.lora_alpha}, dropout={args.lora_dropout}')
        
        # Lock entire text tower first (freeze all base parameters)
        ...
        if is_master(args):
            ...
            from open_clip.lora_utils import count_parameters
            ...
            lora_pct = 100.0 * lora_params / total_params if total_params > 0 else 0
            
            logging.info(f"LoRA applied to {lora_stats} transformer blocks")
            logging.info(f'Total parameters: {total_params:,}')
            logging.info(f'Trainable parameters: {trainable_params:,} ({trainable_pct:.2f}%)')
            logging.info(f'LoRA parameters: {lora_params:,} ({lora_pct:.2f}%)')
            logging.info('=' * 60)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.save_logs:
            ...
            if args.use_lora and args.save_lora_only:
                lora_checkpoint_dict = {
                    "epoch": completed_epoch,
                    "name": args.name,
                    "lora_state_dict": original_model.get_lora_state_dict(),
                    "lora_config": {
                        "r": args.lora_r,
                ...
                lora_checkpoint_path = os.path.join(args.checkpoint_path, f"epoch_{completed_epoch}_lora.pt")
                torch.save(lora_checkpoint_dict, lora_checkpoint_path)
                if is_master(args):
                    lora_size_mb = os.path.getsize(lora_checkpoint_path) / (1024 * 1024)
                    logging.info(f"Saved LoRA adapters to epoch_{completed_epoch}_lora.pt ({lora_size_mb:.2f} MB)")
