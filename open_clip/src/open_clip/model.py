...
class CLIP(nn.Module):
    ...
    def apply_lora_to_text_encoder(
        ...
        """Apply LoRA adapters to text encoder attention layers."""
    ...
    def unmerge_lora(self):
        ...
        from .lora_utils import unmerge_lora as unmerge_lora_fn
        unmerge_lora_fn(self)
    
    def get_lora_state_dict(self):
        """Get only LoRA parameters for saving."""
        from .lora_utils import get_lora_state_dict as get_lora_state_dict_fn
        return get_lora_state_dict_fn(self)
    
    def load_lora_state_dict(self, state_dict, strict: bool = False):
        """Load LoRA parameters."""
        for name, param in state_dict.items():
            if name in dict(self.named_parameters()):
                dict(self.named_parameters())[name].data.copy_(param)
        return {'lora_config': {}}
...
class CustomTextCLIP(nn.Module):
    ...
    def apply_lora_to_text_encoder(
        ...
        """Apply LoRA adapters to text encoder attention layers."""
        from .lora_utils import apply_lora_to_text_encoder as apply_lora_fn
        return apply_lora_fn(self.text, r, lora_alpha, lora_dropout, target_modules)
    
    def enable_lora(self):
        """Enable LoRA adapters."""
        from .lora_utils import enable_lora as enable_lora_fn
        enable_lora_fn(self.text)
    
    def disable_lora(self):
        """Disable LoRA adapters (use base model only)."""
        from .lora_utils import disable_lora as disable_lora_fn
        disable_lora_fn(self.text)
    
    def merge_lora(self):
        """Merge LoRA weights into base model."""
        from .lora_utils import merge_lora as merge_lora_fn
        merge_lora_fn(self.text)
    
    def unmerge_lora(self):
        """Unmerge LoRA weights from base model."""
        from .lora_utils import unmerge_lora as unmerge_lora_fn
        unmerge_lora_fn(self.text)
    
    def get_lora_state_dict(self):
        """Get only LoRA parameters for saving."""
        from .lora_utils import get_lora_state_dict as get_lora_state_dict_fn
        return get_lora_state_dict_fn(self.text)
    
    def load_lora_state_dict(self, state_dict, strict: bool = False):
        """Load LoRA parameters."""
        for name, param in state_dict.items():
            if name in dict(self.text.named_parameters()):
                dict(self.text.named_parameters())[name].data.copy_(param)
        return {'lora_config': {}}
